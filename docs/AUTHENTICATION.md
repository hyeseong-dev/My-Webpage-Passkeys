...
기록 기준: 2026-09-11. 이 프로젝트는 작성자가 공개 범위를 선별한 실제 자기소개와, 패스키로 인증한 계정만 읽을 수 있는 합성 자료를 제공하는 교육용 데모다. 패스키 계정과 비공개 자료는 모두 만들어 넣은 예시이며 연락처와 `/career`의 상세 지원 정보는 포함하지 않는다.
...
AI는 명세 분석, 별도 프로젝트 구성, 기존 공개 소개의 이식, 패스키 화면과 서버 구현, Neon 연결·DB 스키마 적용, 배포, 자동 테스트와 요청·응답 기록, 코드 검토 및 문서 초안 작성을 담당했다.

### 내가 직접 판단한 일

제출자는 Neon PostgreSQL 사용, 고정 도메인 사용, 실제 패스키 인증의 사용자 직접 승인을 결정했다. 공개 자기소개는 본인이 공개해도 된다고 선별한 1번 과제 내용을 사용하고, 패스키 계정과 비공개 자료만 합성 데이터로 구성하기로 판단했다. 기존 `/career`의 연락처·상세 지원 정보는 제외하며, 실제 인증기 승인과 선택한 저장 위치 확인은 제출자가 직접 수행한다.

### AI 제안을 따르지 않은 일

초기에는 C12를 넓게 해석해 공개 소개까지 익명·합성 내용으로 바꾸는 방안을 제안했지만, 제출자는 자기소개 과제의 성격과 C11을 근거로 이를 따르지 않았다. 대신 공개하기로 선별한 실제 소개는 유지하고, 인증 기능의 계정·비공개 기록만 합성 데이터로 한정했다. 문서와 화면에도 이 경계를 명시한다.
